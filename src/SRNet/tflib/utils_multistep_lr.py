...
def train(model_class, train_gen, valid_gen, train_batch_size, \
          ...
          save_interval, start_save_iter, log_path, num_runner_threads=1, \
          load_path=None):
    ...
    with tf.Session() as sess:
        ...
        best_val_acc = _acc_val
        best_iter = 1
        print ("initial accuracy on validation set: {0:.4f}".format(_acc_val))
        print ("evaluation time on validation set: {0:.4f} seconds".format(time.time() - _time))
        ...
        for i in range(start+1, max_iter+1):
            ...
            if i % valid_interval == 0:
                ...
                for j in range(0, valid_ds_size, valid_batch_size):
                    sess.run([increment_valid])
                _acc_val = sess.run(valid_accuracy_s.mean_variable)
                if _acc_val >= best_val_acc:
                    best_val_acc = _acc_val
                    best_iter = i
                    print('update!,best iter:{0}, acc:{1:.4f}'.format(i,_acc_val))
                    saver.save(sess, log_path + '/best_model.ckpt')
                valid_loss_s.add_summary(sess, writer, i)
                valid_accuracy_s.add_summary(sess, writer, i)
                sess.run(enable_training_op)
            if i % save_interval == 0 and i>=start_save_iter:
                saver.save(sess, log_path + '/Model_' + str(i) + '.ckpt')
    return best_iter, best_val_acc
