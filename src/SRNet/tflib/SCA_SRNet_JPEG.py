...
class SCA_SRNet_JPEG(Model):
    def _build_model(self, input_batch, eps=1e-6):
        ...
        with arg_scope([layers.conv2d], num_outputs=16,
            ...
            with tf.variable_scope('Layer1'): # 256*256
                ...
                beta = tf.cast(_inputs_Beta, tf.float32) + eps # Plus a small value to avoid NaN gradient
                prob_map = tf.sqrt(tf.nn.conv2d(beta, tf.abs(W), [1,1,1,1], 'SAME', \
                        data_format=self.data_format))
